		...
		try {
			...
			toast.promise(addPromise, {
				...
				error: (error) => {
					...
					return `Failed to create supplier: ${error.response?.data?.message || error.message
						}`;
				},
			});
		...
		try {
			...
			toast.promise(updatePromise, {
				...
				error: (error) => {
					...
					return `Failed to update supplier: ${error.response?.data?.message || error.message
						}`;
				},
			});
		...
		try {
			...
			toast.promise(deletePromise, {
				...
				error: (error) => {
					...
					return `Failed to delete supplier: ${error.response?.data?.message || error.message
						}`;
				},
			});
		} catch (error) {
	...
	return (
		<div className="bg-white/80 backdrop-blur-sm rounded-2xl p-6 shadow-xs border border-gray-100/80 hover:shadow-md hover:border-gray-200/60 transition-all duration-300 group">
			...
			<div className="flex items-center justify-between mb-4">
				<div className="flex items-center gap-2">
					<div
						className={`h-2 w-2 rounded-full ${supplier.leadTime <= 3
							? "bg-green-500"
							: supplier.leadTime <= 7
								? "bg-yellow-500"
								: "bg-red-500"
